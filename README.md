...
Transcripts are append-only: between two runs a handful of them grow and the rest are byte-for-byte identical. The scan cache in `.cache/` keeps the merged session state from the last run, so a repeat run only re-reads the transcripts that actually moved and only re-renders the session pages whose content changed. On a corpus of ~8,700 transcripts that turns a two-minute run into roughly half a minute.
